...
apt install -y samba-ad-dc krb5-user smbclient winbind auditd lynis audispd-plugins fail2ban sudo ufw krb5-admin-server dnsutils openssh-server | tee -a "$LOG_FILE"

# Vérification des paquets installés
for pkg in samba-ad-dc krb5-user smbclient winbind auditd lynis audispd-plugins fail2ban sudo ufw krb5-admin-server dnsutils openssh-server; do
